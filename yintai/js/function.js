...
function loucengtiaozhuan(btnBoxD,btnBoxlou,floor,topnav,color,activecolor,fhtop){
	const btnBox=$(btnBoxD)[0];
	const lis=$(btnBoxlou);
	const floors=$(floor);
	const nav=$(topnav)[0];
	const fhtops=$(fhtop)[0];
	let winH=document.documentElement.clientHeight;
	let moveH=winH/2-parseInt(window.getComputedStyle(floors[0],null).height)/2;
	let scrObj=scrollobj();
	for(let i=0;i<lis.length;i++){
		lis[i].onclick=function(){
			animate(scrObj,{scrollTop:floors[i].offsetTop+200},500,function(){});
		}
	}
    fhtops.onclick=function(){
        animate(scrObj,{scrollTop:0},500,function(){});
    }
	...
	window.onscroll=function(){
		for(let i=0;i<floors.length;i++){
			if((scrObj.scrollTop)>=floors[i].offsetTop){
				for(let j=1;j<lis.length;j++){
					lis[j].classList.remove(color);
				}
				lis[i].classList.add(activecolor);
			}
		}
		if(scrObj.scrollTop>=750){
			if(flagx){
				flagx=false;
				flags=true;
