...
function loucengtiaozhuan(btnBoxD,btnBoxlou,floor,topnav,color,activecolor){
	...
	for(let i=0;i<lis.length;i++){
		lis[i].onclick=function(){
			...
			animate(scrObj,{scrollTop:floors[i].offsetTop-moveH},1000,function(){});
		}
		// lis[i].onmouseover=function(){
	...
	window.onscroll=function(){
		for(let i=0;i<floors.length;i++){
			if((scrObj.scrollTop+0.8*winH)>=floors[i].offsetTop){
		...
		if(scrObj.scrollTop>=500){
			if(flagx){
				...
				if(scrObj.scrollTop>=(floors[0].offsetTop-300)){
					animate(btnBox.parentNode,{opacity:1},500);
				}else{
					animate(btnBox.parentNode,{opacity:0},500);
				}
